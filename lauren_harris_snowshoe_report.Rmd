...
ggplot(data = juv_hares_sex_site, aes(x = sex, y = weight, color = sex)) +
  ...
  scale_color_manual(values = c("lightpink2", "skyblue", "plum")) +
...
**Figure 2.** Distribution of body weights g(g) for juvenile hares from Bonanza Creek Experimental Forest LTER site (`r min(juvenile_hares$year)`-`r max(juvenile_hares$year)`). Hares are grouped by sex and site. Individual data points are shown with hollow points and outliers are denoted by black points. The middle line is .Box endpoints are the 25th and 75th percentiles. Data: Kielland et al. 2017. 
...
juv_hares_sex_site_stats %>% 
  kable(col.names = c("Sex", "Site", "Mean (g)", "Standard Deviation (g)", "Sample Size"), digits = 2) %>% 
