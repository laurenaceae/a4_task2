...
The mean weight of juvenile female hares is `r round(f_juv_mean, 2)` g, which is `r round((1-(f_juv_mean/m_juv_mean))*100, 2)`% smaller than the mean weight of juvenile male hares at `r round(m_juv_mean, 2)` g. The actual difference in mean weights is `r round(m_juv_mean - f_juv_mean, 2)` g. The effect of sex on body weight is not very large (d = `r round(juv_hares_cohen$estimate, 3)`) so the means are relatively close together. However, a t-test (Unpaired `r juv_hares_ttest$method`: p-value = `r round(juv_hares_ttest$p.value, 3)`, t(`r round(juv_hares_ttest$parameter, 2)`) = `r round(juv_hares_ttest$statistic, 2)`) shows that the means are significantly different. This t-test purported a `r round((juv_hares_ttest$p.value)*100, 2)`% chance of finding means at least this different, if they are drawn from populations with the same mean. Diagnostic plots (histograms, qq plots) showed data and residuals were normally distributed. 
